...
chosen_model = model_list["A"]
model_class = chosen_model["class"]
saved = chosen_model["saved"]
tok_file = chosen_model["tokenizer"]
...
# Keep chat history and hidden state across reruns
...
def generate_reply(user_input):
    with torch.inference_mode():
        input_tensor = torch.tensor(tokenizer.encode(user_input), dtype=torch.long).unsqueeze(0)
        out, state = model(input_tensor, st.session_state.state)
...
# Chat input box
user_input = st.chat_input("Say something to Snortle AI...")
